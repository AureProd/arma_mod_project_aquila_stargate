...
if ((typeOf _gate) == "PA_stargate_tauri") then {
	...
	_gate setObjectTexture [20, "\sgi_porte_des_etoiles\tex\glyphs.paa"];
	_gate setObjectTexture [21, "\sgi_porte_des_etoiles\tex\innerr.paa"];
} else {
	[_gate, false] call _light;
	
	_gate setObjectTexture [10, "\sgi_porte_des_etoiles\Texture\SGI_portedesetoiles_co.paa"];
};
